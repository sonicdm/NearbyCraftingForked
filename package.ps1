...
<#
.SYNOPSIS
  Builds (optional) and packs Nearby Crafting Forked for Thunderstore / r2modman.
...
.PARAMETER SkipBuild
  Package the existing bin\<Configuration>\NearbyCraftingForked.dll without rebuilding.
...
$Dll = Join-Path $ProjectRoot "bin\$Configuration\NearbyCraftingForked.dll"
...
$PluginSrc = Join-Path $ProjectRoot "src\NearbyCraftingForked.cs"
$CsprojPath = Join-Path $ProjectRoot "NearbyCraftingForked.csproj"
...
Assert-True ($pluginSrcRaw -match 'PluginVersion\s*=\s*"(\d+\.\d+\.\d+)"') `
    "Could not find PluginVersion in NearbyCraftingForked.cs"
...
Assert-True ($csprojRaw -match '<Version>(\d+\.\d+\.\d+)</Version>') `
    "Could not find <Version> in NearbyCraftingForked.csproj"
...
Copy-Item -LiteralPath $Dll -Destination (Join-Path $Stage "NearbyCraftingForked.dll")
...
try {
    ...
    $requiredInZip = @("manifest.json", "README.md", "icon.png", "NearbyCraftingForked.dll")
...
Copy-Item -LiteralPath $Dll -Destination (Join-Path $OutDir "NearbyCraftingForked.dll") -Force
...
Write-Host "Or import the zip into r2modman as a local mod (installs as SonicDM-NearbyCraftingForked)."
